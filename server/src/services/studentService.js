...
export async function createStudent(data) {
  ...
  logger.info(`新增学生: ${name}, 学号: ${studentNo}`);
...
export async function updateStudent(id, data, user) {
  ...
  if (user && user.role === 'teacher') {
    ...
    logger.info(`班主任更新学生信息: ${result.rows[0].name}`);
    return result.rows[0];
  }
  ...
  logger.info(`更新学生: ${result.rows[0].name}`);
  return result.rows[0];
}
...
export async function deleteStudent(id) {
  ...
  logger.info(`删除学生: ${result.rows[0].name}`);
  return { deleted: true };
}
...
export async function changeStudentStatus(id, data, user) {
  ...
  logger.info(`学籍变更: ${studentName} ${fromStatus} → ${status}, 原因: ${reason || '无'}`);
