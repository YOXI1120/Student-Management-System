...
export async function getAnnouncements(req, res) {
  try {
    ...
    logger.error('查询公告列表出错:', err.message);
...
export async function getLatestAnnouncements(req, res) {
  try {
    ...
    logger.error('查询最新公告出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
...
export async function createAnnouncement(req, res) {
  try {
    ...
    logger.error('发布公告出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
...
export async function updateAnnouncement(req, res) {
  try {
    ...
    logger.error('编辑公告出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
...
export async function deleteAnnouncement(req, res) {
  try {
    ...
    logger.error('删除公告出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
