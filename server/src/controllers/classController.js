...
export async function getClasses(req, res) {
  try {
    ...
    logger.error('查询班级列表出错:', err.message);
...
export async function getClassById(req, res) {
  try {
    ...
    logger.error('查询班级详情出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
...
export async function createClass(req, res) {
  try {
    ...
    logger.error('创建班级出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
...
export async function updateClass(req, res) {
  try {
    ...
    logger.error('修改班级出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
...
export async function deleteClass(req, res) {
  try {
    ...
    logger.error('删除班级出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
