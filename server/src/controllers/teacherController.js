...
export async function getTeachers(req, res) {
  try {
    ...
    logger.error('查询教师列表出错:', err.message);
...
export async function getTeacherById(req, res) {
  try {
    ...
    logger.error('查询教师详情出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
...
export async function createTeacher(req, res) {
  try {
    ...
    logger.error('创建教师出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
...
export async function updateTeacher(req, res) {
  try {
    ...
    logger.error('修改教师出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
...
export async function deleteTeacher(req, res) {
  try {
    ...
    logger.error('删除教师出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
