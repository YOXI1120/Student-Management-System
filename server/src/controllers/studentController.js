...
export async function getStudents(req, res) {
  try {
    ...
    console.error('查询学生列表出错:', err.message);
...
export async function getStudentById(req, res) {
  try {
    ...
    console.error('查询学生详情出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
...
export async function createStudent(req, res) {
  try {
    ...
    console.error('创建学生出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
...
export async function updateStudent(req, res) {
  try {
    ...
    console.error('修改学生出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
...
export async function deleteStudent(req, res) {
  try {
    ...
    console.error('删除学生出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
...
export async function changeStudentStatus(req, res) {
  try {
    ...
    console.error('变更学生状态出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
...
export async function getStatusLogs(req, res) {
  try {
    ...
    console.error('查询学籍变更记录出错:', err.message);
    return res.status(500).json(Response.error(500, '服务器内部错误'));
  }
}
